...
module ::OmniAuth
  module OpenIDConnectRBX
    class DiscoveryError < Error
    end
  end

  module Strategies
    class OpenIDConnectRBX < OmniAuth::Strategies::OAuth2
      ...
      def discover!
        ...
        if discovery_document.nil?
          raise ::OmniAuth::OpenIDConnectRBX::DiscoveryError.new("Discovery document is missing")
        end
        ...
        discovery_params.each do |internal_key, external_key|
          ...
          if val.nil? || val.empty?
            raise ::OmniAuth::OpenIDConnectRBX::DiscoveryError.new(
      ...
      def request_phase
        ...
        rescue ::OmniAuth::OpenIDConnectRBX::DiscoveryError => e
      ...
      def userinfo_response
        ...
        if userinfo_sub != id_token_sub
          raise SubVerifyError.new(
                  "RBXOIDC `sub` mismatch. ID Token value: #{id_token_sub.inspect}. UserInfo value: #{userinfo_sub.inspect}",
                )
        end
        @raw_info
      end
...
OmniAuth.config.add_camelization "openid_connect_rbx", "OpenIDConnectRBX"
